...
def read_http_response(remote_socket, request_method):
    ...
    response_version, status_code = parts[0], int(parts[1])
    body = b""
    ...
    if response_has_body(request_method, status_code):
        if "chunked" in headers.get("transfer-encoding", "").lower():
            ...
            body = read_exact_bytes(remote_socket, body_part, int(headers["content-length"]))
        else:
...
def handle_http_request(client_socket, client_address, method, target, version, headers, body, origin_connections):
    ...
    if cache.should_cache_request(method):
        ...
        if entry:
            ...
            client_socket.sendall(entry.response_bytes)
            return False # client keeps connection if requested
    ...
    for attempt in range(2):  # one retry with a fresh socket
        ...
        try:
            ...
            res_bytes = build_client_response(status_line, res_headers, res_body, True, method, status_code)
            
            # Part F: Store in cache (Talia)
            if cache.should_cache_request(method):
                cache.put(cache_key, res_bytes)
    ...
    try:
        ...
        while True:
            ...
            if method == "CONNECT":
                ...
                handle_https_connect(client_socket, client_address, *parse_host_port(target, 443), logger=logger)
                break
